...
# downsignificantly by the fact that we're doing it on a pandas dataframe. If we
# work with a dictionary instead, we can use the `update` function to speed up
# our indexing by orders of magnitude.
#
...
sparse_0 = {f"{r.key}_{r.alliance[0]}":dict(row) for i,r in data.iterrows()}
...
def build_sparse(row):
    sparse_0[f"{row.key}_{row.alliance[0]}"].update(dict(zip(row.teams, [1,1,1])))

data.apply(build_sparse, axis=1)

# %%
# Deconstruct sparse into matrix
sparse_m = np.array([list(d.values()) for d in sparse_0.values()])
sparse_m
...
# except for when teams have standout performances.
